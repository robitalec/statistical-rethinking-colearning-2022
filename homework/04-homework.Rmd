...
```{r h04_q02_loo}
message('Model 1, Direct: Weight ~ Food + Group Size')
tar_read(loo_h04_q02_direct)

message('Model 2, Total: Weight ~ Food')
tar_read(loo_h04_q02_total)

message('Compared')
tar_read(compare_h04_q02)
```

Based on the PSIS LOO, model 2 has better predictive power than model 1. 
Model 1 includes both food and group size to model the direct effect of food
on weight. Model 2 includes only food to measure the total effect of food
on weight. The model with better predictive power (model 2), however, 
indicates no

```{r h04_q02_params}
message('Model 1, Direct: Weight ~ Food + Group Size')
plot(precis(data.table(tar_read(h03_q02_draws_h03_q02_direct))[, .(sigma, alpha, beta_food, beta_groupsize)], depth = 2))

message('Model 2, Total: Weight ~ Food')
plot(precis(data.table(tar_read(h03_q02_draws_h03_q02_total))[, .(sigma, alpha, beta_food)]))
```



The model including both food and group size to measure only the direct 
effect of food on weight (model 1) 